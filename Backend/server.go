...
import (
	...
	"github.com/tkanos/gonfig"         // config file handler
)
...
func dbAddNewNode(parentID int, Title string, Avatar string) (int, string, error) {
	tx, err := db.Begin()
	if err != nil {
		log.Fatal(err)
	}
	...
	if Avatar == "" {
		Avatar = configuration.Default_avatar
	}
	...
	result, err := tx.Exec(`INSERT INTO NodesTable(Title, lft, rgt, Avatar)
...
func addNodeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if fileStatus := r.FormValue("filestatus"); fileStatus == "true" {
		...
		path := configuration.Avatars_folder + fileName
		fileCounter += 1
	...
	if m == nil || data.Title == "" {
		data.Title = configuration.Default_title
	}

	// Add node to the database.
	newId, newTitle, err := dbAddNewNode(data.ParentID, data.Title, fileName)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		fmt.Println(err)
		return
	}
	// Return info about New Node.
	NewNode := Node{ID: newId, Title: newTitle}
	enc := json.NewEncoder(w)
	err = enc.Encode(&NewNode)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		fmt.Println(err)
		return
...
func getAvatarHandler(w http.ResponseWriter, r *http.Request) {
	...
	avatarPath := configuration.Avatars_folder + fileName
...
func main() {
	...
	log.Println("Server has started -> http://localhost:" + configuration.Port)
